...
**Preventive Controls:**
- **Least Privilege Tools**: Each tool granted minimum required permissions; separate read/write tools. See [[mitigations/least-privilege-implementation|Least Privilege Tool Implementation]] for architectural guidance.
- **Input Validation**: Strict whitelisting of tool arguments; reject injection patterns. See [[mitigations/input-validation-patterns|Input Validation Patterns]] for validation approaches.
...
**Detective Controls:**
- **Behavioral Anomaly Detection**: Alert on unusual tool usage patterns (time, frequency, sequence). See [[mitigations/anomaly-detection-architecture|Anomaly Detection Architecture]] for detection patterns.
...
- **Mitigated by**: [[mitigations/least-privilege-implementation]], [[mitigations/tool-sandboxing-architecture]], [[mitigations/approval-workflow-patterns]], [[mitigations/anomaly-detection-architecture]]
- **Enables**: [[techniques/unsafe-tool-invocation]]
- **Enabled by**: [[techniques/prompt-injection]], [[techniques/agent-goal-hijack]]
- **ATLAS**: AML.T0051
